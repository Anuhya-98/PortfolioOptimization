...
CREATE OR REPLACE VIEW public.v_eod_indices_2016_2020 AS
 SELECT eod_indices.symbol,
   ...
   FROM eod_indices
   WHERE eod_indices.date >= '2016-01-01'::date AND eod_indices.date <= '2020-12-31'::date;
...
FROM custom_calendar CC INNER JOIN v_eod_indices_2016_2020 II ON CC.date=II.date
GROUP BY symbol,y,m
ORDER BY symbol,y,m;
...
/*
-- LIFELINE:
ALTER TABLE public.exclusions_2016_2020
    ADD CONSTRAINT exclusions_2016_2020_pkey PRIMARY KEY (ticker);
*/
...
-- CHECK:
SELECT * FROM v_eod_2016_2020; -- slow
...
-- CHECK
SELECT * FROM mv_eod_2016_2020; -- faster
...
SELECT * FROM mv_eod_2016_2020 WHERE symbol='GWPH';
SELECT * FROM mv_ret_2016_2020 WHERE symbol='GWPH' ORDER BY ret DESC;
-- IMPORTANT: we have stored (materialized) views, we need to refresh them IN A SEQUENCE!
REFRESH MATERIALIZED VIEW mv_eod_2016_2020 WITH DATA;
-- CHECK:
SELECT * FROM mv_eod_2016_2020 WHERE symbol='GWPH'; -- excluded

REFRESH MATERIALIZED VIEW mv_ret_2016_2020 WITH DATA;
-- CHECK:
SELECT * FROM mv_ret_2016_2020 WHERE symbol='GWPH'; -- excluded
...
FROM
(SELECT DISTINCT symbol FROM mv_eod_2016_2020) S
